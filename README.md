...
- 新增规则源时，只需要补充 workflow 里的下载 URL 和 README 中的对应说明；不需要新增生成脚本、额外目录层级或二次处理流程。
- 移除规则源时，只需要删除对应 URL 与文档条目，不需要改动额外的生成链路。
- 切换上游 URL 时，不需要重构仓库结构；继续沿用现有 `rules/` 输出目录与配置引用方式即可。
- 不要把这个仓库扩展成复杂的规则生成系统；它只负责维护一组可追溯的上游 URL、把原始规则同步到 `rules/`，并让 Clash Verge / Mihomo 直接引用这些结果。
